...
def CostFunction(theta, X, Y, lmbd):
    ...
    reg_term = lmbd * np.sum(np.square(theta)[1:theta.shape[0]+1])/(2*X.shape[0])  #mind that we don't have to include theta0
    cost = J + reg_term
    return cost
    ...
    if X.shape[1] == 3:
        ...
        for i in range(0, u.size):
            for j in range(0, v.size):
                z[i,j] = np.dot(MapFeature(np.mat(u[i]),np.mat(v[j]),7),theta)
